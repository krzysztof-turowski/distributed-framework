...
func (v *twoWayNode) ReceiveMessageWithTimeout(index int) []byte {
	neighborsCasesTimeout := make([]reflect.SelectCase, 2)
	neighborsCasesTimeout[0] = v.neighborsCases[index]
...
func (v *twoWayNode) IgnoreFutureMessages() {
	go func() {
		for {
			_, _, _ = reflect.Select(v.neighborsCases)
		}
	}()
}
