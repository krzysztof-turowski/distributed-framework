...
func run(v lib.Node, rounds int) {
	v.StartProcessing()
...
func check(vertices []lib.Node) {
	...
	for _, v := range vertices {
		s := getStateMesh(v)
		if s.Status == leader && leadNode != nil {
	...
	for _, v := range vertices {
		...
		if s.Leader != leadNode.GetIndex() {
			panic("node has a wrong leader")
		}
	}
	log.Println("all good, leader id is: ", leadNode.GetIndex())
}

func Run(vertices []lib.Node, synchronizer lib.Synchronizer, rounds int) {
	for _, v := range vertices {
