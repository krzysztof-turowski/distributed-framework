...
func processRound(v lib.Node) bool {
	...
	sendAllBenOr(v, messageBenOr{Type: messageTypeOne, Value: state.EndingBit, AlreadyFinished: state.Done})
...
func initializeBenOr(v lib.Node, startingBit byte, n, t int) bool {
	setState(v, &stateBenOr{
...
func runBenOr(v lib.Node, startingBit byte, n, t int) {
	v.StartProcessing()
	finish := initializeBenOr(v, startingBit, n, t)
...
func RunBenOr(processes []byte, behaviours []func(r int) byte) {
	...
	for i := 0; i < n-t; i++ {
		...
		go runBenOr(nodes[i], bit, n, t)
	}

	for i := 0; i < t; i++ {
		go runFaultyBenOr(nodes[n-t+i], behaviours[i], n, t)
	}
...
func processFaultyRound(v lib.Node, r int, behaviour func(r int) byte) bool {
	...
	sendAllBenOr(v, messageBenOr{Type: messageTypeOne, Value: behaviour(r)})
	receiveAndProcessMessages(v)

	sendAllBenOr(v, messageBenOr{Type: messageTypeTwo, Value: behaviour(r), Decided: true})
	receiveAndProcessMessages(v)
...
func runFaultyBenOr(v lib.Node, behaviour func(r int) byte, n, t int) {
	v.StartProcessing()
	finished := initializeBenOr(v, behaviour(0), n, t)
...
func sendAllBenOr(node lib.Node, message messageBenOr) {
