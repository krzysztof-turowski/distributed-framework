...
func GetFaultyBehavior(nodes []lib.Node, faultyIndices map[int]int, strategy Strategy) func(lib.Node) bool {
	...
	return func(node lib.Node) bool {
		...
		case ER0:
			...
			s.exchangeRound = ER1
		case ER1:
			strategy.er1(node, nodes, faultyIndices, receive(node))
			s.exchangeRound = ER2
		case ER2:
			msgs := receive(node)
			if s.phase == node.GetIndex() {
				strategy.er2(node, nodes, faultyIndices, msgs)
			} else {
				broadcastEmpty(node)
			}
			s.exchangeRound = ER3
...
type Strategy interface {
	...
	er2(node lib.Node, nodes []lib.Node, faultyIndices map[int]int, msgs []*Message)
}
