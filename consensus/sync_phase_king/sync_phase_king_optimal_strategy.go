...
func (r *Optimal) er0(v lib.Node, nodes []lib.Node, faultyIndices map[int]int, _ []*Message) {
	...
	if j := faultyIndices[v.GetIndex()]; j+E < len(nodes)-len(faultyIndices) {
...
func (r *Optimal) er1(v lib.Node, _ []lib.Node, _ map[int]int, _ []*Message) {
	broadcast(v, &Message{V: 2})
}

func (r *Optimal) er2(v lib.Node, _ []lib.Node, faultyIndices map[int]int, _ []*Message) {
